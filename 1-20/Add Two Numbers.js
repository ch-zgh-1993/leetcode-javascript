...
* @Last Modified time: 2018-09-28 15:35:38
...
// 
// 尝试思路：
// ---若是单链表，则需要找出每一个值，存储在 arr 中，再将两个 arr 相加。但看示例提供的参数是数组list。
// 
// ---最直接的方法，创建第三个list，以长度较大的作为循环长度，倒序相加，将至存在第三个list中，若相加超过10，则可临时存储，在下次求和时加上，清空临时值，再判断即可。加完后对第
// 三个 list 求反即可。 但是返回的是 [], 因为给的并不是数组，而是对象.....能不能不要这么误导...一波三折。没关系，其实这个理解做了中间的运算求结果部分。
// 
// 1. 给出的是一个单链表，那么将两个链表的值存放在arr中，创建第三个链表，对求和结果进行保存，返回第三个链表。中间运行部分依据错误理解即可。 O(n)
// 
